  ...
  <Overflow v-else>
    ...
    <div class="overflow__content">
      <Form class="form" @submit="onSubmit">
        ...
        <div>
          <div class="text text_normal text_center text_light">
            Оставляя данные в этой форме Вы даете
          </div>
        ...
        <button :disabled="disabled" class="button button_gradient button_fill">
          Отправить
        </button>
      </Form>
    </div>
...
async function onSubmit(values) {
  ...
  if (file.value !== null) {
    fileError.value = "";
    const { data, status } = await sendFormVacancies(
      values.fam,
      values.name,
      values.phone,
      props?.vacancy.title,
